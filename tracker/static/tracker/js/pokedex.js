...
function filterCards() {
    ...
    cards.forEach(card => {
        ...
        const isCaught = card.dataset.caught === 'true';
        ...
        if (data.success) {
            ...
            if (card) {
                ...
                if (data.is_caught) {
                    ...
                    if (statusBadge) {
                        statusBadge.innerHTML = '<span class="status-check-box w-2.5 h-2.5 rounded-sm border border-current flex items-center justify-center text-[8px] leading-none shrink-0">✓</span><span class="status-text font-black">Atrapado</span>';
                        statusBadge.className = 'card-status-indicator h-6 min-w-[78px] flex items-center justify-center gap-1 px-1.5 rounded-md text-[10px] font-black uppercase tracking-wider border-2 border-slate-950 shadow-[1px_1px_0px_#0f172a] shrink-0 ' + THEME_STATUS_CAUGHT_CLASS;
                    }
                } else {
            ...
            // Sincronizar con la tarjeta de transferencias si este Pokémon está en ella
            ...
            if (statCaught) statCaught.textContent = data.caught_count;
            if (statPercent) statPercent.textContent = data.percent;
...
function openPokemonModal(entryId) {
    ...
    // 2. Sprite según estilo activo (retro o modern)
    const modalImg = document.getElementById('modal-pokemon-img');
    if (modalImg) {
    ...
    const card = document.getElementById(`card-${entryId}`);
    const isCaught = card ? (card.dataset.caught === 'true') : false;
...
function updateModalCatchStatus(isCaught) {
    ...
    if (badge && btn && text) {
        if (isCaught) {
            badge.className = 'px-2.5 py-1 rounded-md text-xs font-black uppercase tracking-wider border-2 border-slate-950 bg-emerald-400 text-slate-950 shadow-[1.5px_1.5px_0px_0px_#0f172a]';
            badge.textContent = '✓ Atrapado en tu partida';
            ...
            badge.textContent = 'Pendiente de capturar';
