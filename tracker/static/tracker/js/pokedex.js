...
function applyShinydexMode(active) {
    ...
    if (typeof updateUnownModalUI === 'function') {
        updateUnownModalUI();
    }

    if (typeof updateExclusivesModalUI === 'function') {
...
function openClassicUnownModal() {
    closeUnownModal();
...
function switchToUnownModal() {
    closePokemonModal();
    openUnownModal();
}

function openPokemonModalDirect(entryId) {
    ...
    // 7. Botones de retorno al Bloc Unown (solo presentes al ver a Unown)
    ...
    // 9. Activar marquesina suave en ubicaciones cuyo texto desborde
...
function updateExclusivesProgressUI() {
    ...
    counterpartCards.forEach(c => {
        const isCaught = isShinydexMode ? (c.dataset.shinyCaught === 'true') : (c.dataset.caught === 'true');
    ...
    const themeColorClass = (counterpartTheme === 'blue') ? 'text-blue-600' :
        ...
        ((counterpartTheme === 'gold') ? 'text-[#9e7514]' : 'text-red-600')));
    const themeBarClass = (counterpartTheme === 'blue') ? 'bg-blue-600' :
        ...
        ((counterpartTheme === 'gold') ? 'bg-[#c59b27]' : 'bg-red-600')));

    const caughtElem = document.getElementById('excl-progress-caught');
    if (caughtElem) {
        caughtElem.textContent = caughtCount;
        caughtElem.className = (isShinydexMode ? 'text-amber-600' : themeColorClass) + ' font-black';
    }
    ...
    if (barElem) {
        ...
        barElem.className = isShinydexMode 
...
/* ===== Bloc Unown / Unowndex Controller ===== */
function openUnownModal() {
...
function filterUnownChamber(chamberKey) {
...
function updateUnownModalUI() {
    ...
    const curStats = isShinydexMode ? unownStats.shiny : unownStats.normal;
    if (title) title.textContent = isShinydexMode ? 'Progreso Unowndex Shiny:' : 'Progreso Unowndex:';
    ...
    if (progressBar) {
        ...
        progressBar.className = isShinydexMode
            ? 'bg-gradient-to-r from-amber-400 to-yellow-300 shadow-[0_0_8px_rgba(251,191,36,0.6)] h-full rounded-full transition-all duration-300'
    ...
    document.querySelectorAll('.unown-card').forEach(card => {
        ...
        if (img) {
            img.src = isShinydexMode ? img.dataset.spriteShiny : img.dataset.spriteNormal;
        }
...
async function toggleUnownFormCatch(letter) {
    ...
    try {
        ...
        unownStats.normal.caught = res.unown_normal_count;
        unownStats.normal.percent = res.unown_normal_percent;
        unownStats.shiny.caught = res.unown_shiny_count;
        unownStats.shiny.percent = res.unown_shiny_percent;
        updateUnownModalUI();
        ...
        updateCardUI(unownCardMain, isShinydexMode ? res.entry_is_shiny : res.entry_is_caught);

        // 4. Sincronizar progreso general de la Pokédex
        normalStats.caught = res.global_normal_caught;
        normalStats.percent = res.global_normal_percent;
        shinyStats.caught = res.global_shiny_caught;
        shinyStats.percent = res.global_shiny_percent;
        updateProgressBarUI();
